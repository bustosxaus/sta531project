...
# Function that creates formulas for use in model.matrix function
# Input variable names that you want to run in model
...
CV = function(k, pitcher, vars){
  ...
  for(i in 1:k)
  {
    ...
    # run model
    multi_out = mlogit(Y, X_train, n = rep(1, nrow(Y)), samp = 100, burn = 20)
    betas = multi_out$beta
...
vars = c("inning", "pre_outs", "bat_side", "runners", 
         "count", "previous_pitch_type")
